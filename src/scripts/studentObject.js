...
const student = {
    ...
    studyTime: function (f) {
        ...
        let hoursStudied = Math.floor(f / 60);
        // for every hour, increase grades by 5%
        // for every hour, decrease energy by 2
        if (hoursStudied > 0) {
            student.grades = student.grades * (1 + (0.05 * hoursStudied));
            student.energy = student.energy - (2 * hoursStudied);
        }
    }
}
...
console.log(student.grades, student.energy);
student.studyTime(187);
console.log(student.grades, student.energy);